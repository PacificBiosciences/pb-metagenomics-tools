        ...
        if db in exceptions:
            ...
            for sample in samples:
                ...
                if not os.path.exists(fpath):
                    os.mkdir(fpath)
                plt.savefig(os.path.join(fpath, "{}.{}.histo.pdf".format(sample, db)))
                plt.close()
...
def write_main_output(samples, annotations_dict, normalization_dict, outname):
    ...
    with open(outname, 'a') as fh:
        ...
        for db in dbs:
            ...
            fh.write("{}\t{}\t{}\n".format("Number of classes", db, '\t'.join(classes)))
...
def make_plot_dicts(samples, annotations_dict, normalization_dict, outdir):
    ...
    convert_to_df_and_plot(tax_read_dict, ['Sample', 'Total_Reads_Assigned', 'Database'],
                           ...
                           os.path.join(fpath, "SummaryPlot-Taxonomic_Total_Reads_Assigned.nuc.pdf"))
    convert_to_df_and_plot(tax_perc_dict, ['Sample', 'Percent_Reads_Assigned', 'Database'],
                           ...
                           os.path.join(fpath, "SummaryPlot-Taxonomic_Percent_Reads_Assigned.nuc.pdf"))
    convert_to_df_and_plot(tax_class_dict, ['Sample', 'Classes', 'Database'],
                           "Count", "Classes Represented in  Taxonomic Databases",
                           os.path.join(fpath, "SummaryPlot-Taxonomic_Classes_Assigned.nuc.pdf"))
...
def normalize_count_summaries(summaries, normalization_dict, samples, outdir):
    for f in summaries:
        ...
        output = os.path.join(outdir, "Normalized.{}.counts.nuc.txt".format(db))
...
def main():
    ...
    add_summary_info(annotations_dict, samples, args.summaries)
