...
**Snakemake v8+ is now required**. Please note there are some major command-line changes for v8+. You can install snakemake 8.25+ via the provided conda environment file via `conda env create -f snakemake-environment.yml`, and then activate this environment via `conda activate pb-metagenomics-tools` to run the workflows. The new syntax is provided in the documentation below, for each of the workflows.
