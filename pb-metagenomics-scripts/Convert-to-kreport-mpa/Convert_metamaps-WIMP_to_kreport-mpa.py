...
def get_args():
    ...
    parser = argparse.ArgumentParser(
        prog='Convert_metamaps-WIMP_to_kreport-mpa.py',
        description="""Convert a WIMP file obtained from metamaps 
        into an mpa (metaphlan) and kreport (kraken) output format.""")

    parser.add_argument("-i", "--input",
                        required=True,
                        help="A WIMP file obtained from metamaps.")
    ...
    parser.add_argument("-r", "--readcount",
                        required=True,
                        type=int,
                        help="The total number of reads that were screened to produce the WIMP file.")
...
def get_filtered_dataframe(infile):
    # cols = AnalysisLevel	taxonID	Name	Absolute	EMFrequency	PotFrequency
    print("\nReading in WIMP file...")
    df = pd.read_csv(infile, sep='\t')
    print("Total entries = {}".format(df.shape[0]))
    # only counts at species level represent true level counts, all else above is cumulative
    # remove everything above species, and we can build from there
    filt_df = df[(df['AnalysisLevel'] == 'species') & (df['taxonID'] != '0') & (df['taxonID'] != '-3')]
    print("Species entries to use = {}".format(filt_df.shape[0]))
    return filt_df
...
def main():
    ...
    df = get_filtered_dataframe(args.input)
    ...
    for k, v in taxon_dict.items():
        ...
        # add read counts
        v['Count'] = int((df.loc[df['Name'] == k])['Absolute'])
    ...
    write_mpa(expanded_dict, "{}.metamaps-wimp.mpa.txt".format(args.label))
    write_kreport(expanded_dict, "{}.metamaps-wimp.kreport.txt".format(args.label), args.readcount, ncbi)
    print("\nDone!\n")
