...
def get_args():
    ...
    parser = argparse.ArgumentParser(
        prog='Convert_metamaps-KRONA_to_kreport-mpa.py',
        description="""Convert a KRONA file obtained from metamaps 
        into an mpa (metaphlan) and kreport (kraken) output format.""")

    parser.add_argument("-i", "--input",
                        required=True,
                        help="A KRONA file obtained from metamaps.")
    ...
    parser.add_argument("-r", "--readcount",
                        required=True,
                        type=int,
                        help="The total number of reads that were screened to produce the KRONA file.")
...
def get_taxon_dict(ncbi, df):
    ...
    taxon_dict = {}
    for taxid in list(df['Name'].unique()):
        if taxid != 0:
            taxon_dict[ncbi.get_taxid_translator([taxid])[taxid]] = {'taxid': taxid}
    return taxon_dict
...
def main():
    ...
    print("\nReading in metamaps KRONA file...")
    df = pd.read_csv(args.input, sep='\t', names=['Read', 'Taxid', 'Quality'], header=None)
    taxid_df = df['Taxid'].value_counts().rename_axis('Name').reset_index(name='counts')
    taxon_dict = get_taxon_dict(ncbi, taxid_df)
    print("Getting lineage information for all taxa...")
    for k, v in taxon_dict.items():
        # add lineage ranks and names
        v.update(add_ranks(ncbi, k, v))
        # add read counts
        v['Count'] = int((taxid_df.loc[taxid_df['Name'] == v['taxid']])['counts'])
    ...
    write_mpa(expanded_dict, "{}.metamaps-krona.mpa.txt".format(args.label))
    write_kreport(expanded_dict, "{}.metamaps-krona.kreport.txt".format(args.label), args.readcount, ncbi)
    print("\nDone!\n")
