...
def get_args():
    ...
    parser = argparse.ArgumentParser(
        prog='Convert_MEGAN-NCBI-c2c_to_kreport-mpa.py',
    ...
    parser.add_argument("-c", "--columns",
                        required=True,
                        choices=['two', 'three'],
                        help="Whether the c2c file contains two columns (taxon, count) "
                        "or three columns (rank, taxon, count).")
    ...
    parser.add_argument("-r", "--readcount",
                        required=True,
                        type=int,
                        help="The total number of reads that were screened to produce the c2c file.")
...
def main():
    ...
    print("\nReading in c2c file...")
    if args.columns == 'three':
        df = pd.read_csv(args.input, sep='\t', names=['Level', 'Name', 'Count'], header=None)
    elif args.columns == 'two':
        df = pd.read_csv(args.input, sep='\t', names=['Name', 'Count'], header=None)
    ...
    write_mpa(expanded_dict, "{}.megan-c2c.mpa.txt".format(args.label))
    write_kreport(expanded_dict, "{}.megan-c2c.kreport.txt".format(args.label), args.readcount, ncbi)
    print("\nDone!\n")
